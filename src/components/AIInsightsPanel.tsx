...
import { ShieldAlert, ShieldCheck, TrendingUp, TrendingDown, AlertTriangle, CheckCircle2, XCircle, Info, Zap } from 'lucide-react';
...
export function AIInsightsPanel({ listing }: AIInsightsPanelProps) {
  const analysis = useMemo(() => {
    ...
    const verdict = score >= 70 ? 'Opportunité solide' : score >= 50 ? 'À approfondir' : 'Vigilance requise';
    const verdictColor = score >= 70 ? 'text-emerald-400' : score >= 50 ? 'text-amber-400' : 'text-red-400';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white">Analyse AI</p>
              <p className="text-[10px] text-white/40 font-light">Évaluation automatique du dossier</p>
            </div>
          </div>
        ...
        <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden mb-3">
          <motion.div
            ...
            className={`h-full rounded-full ${analysis.score >= 70 ? 'bg-emerald-400' : analysis.score >= 50 ? 'bg-amber-400' : 'bg-red-400'}`}
          />
        </div>
      ...
      <p className="text-[9px] text-white/20 font-light text-center leading-relaxed pt-2">
        Analyse générée automatiquement à partir des données déclarées. Ne constitue pas un conseil en investissement.
      </p>
    </div>
  );
}
